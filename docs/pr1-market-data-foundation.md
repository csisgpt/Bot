...
- `CANDLE_AGGREGATE_CONCURRENCY` هم‌زمانی تجمیع
...
## سیاست کندل بسته
- اینجست فقط کندل‌هایی را ذخیره می‌کند که بسته شده باشند.
- اگر `closeTime` بزرگ‌تر از زمان فعلی (با گارد ۱ ثانیه) باشد، کندل ذخیره نمی‌شود.

## هم‌زمانی تجمیع
- تجمیع تایم‌فریم‌ها با یک استخر هم‌زمانی اجرا می‌شود تا فشار روی دیتابیس کنترل شود.
